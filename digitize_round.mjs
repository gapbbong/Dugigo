...
Return ONLY a valid JSON array of objects. Each object must follow this schema:
{
  ...
  "round": "${round}",
  "question_num": number,
  "question": "text (use LaTeX for math/units like $V$, $A$, $[\\Omega]$, $[H]$)",
  "options": ["1번", "2번", "3번", "4번"],
  "answer": number (1-4),
  "explanation": "brief technical explanation with LaTeX formulas",
  "level": "하/중/상"
}

Important:
1. Use $ formula $ for ALL mathematical expressions and units.
2. If there's a diagram, just put [그림 참고] in the question and I will handle images later.
3. Ensure "answer" is an integer 1-4.
4. Explanations should be helpful and include formulas used.
5. Provide the output as a raw JSON array. No markdown code blocks.`;
...
// Start with 2019 Round 01 as a test
digitizeRound('2019', '01').catch(console.error);
