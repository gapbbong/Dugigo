...
// Mock classification logic from routes.ts
function classify(subject, q) {
  ...
  // 정보처리기능사는 별도 매핑 없이 q.sub_unit 반환
...
subjects.forEach(sub => {
  ...
  console.log(`\n========================================`);
  console.log(`🔍 Testing Subject: ${sub.name}`);
  ...
  Object.keys(groups).sort().forEach(unit => {
    console.log(`   - ${unit}: ${groups[unit]} questions`);
  });

  // Verification checks
  if (sub.name === '컴퓨터활용능력 2급') {
    const hasSubject1 = Object.keys(groups).some(u => u.includes('[1과목]'));
    const hasSubject2 = Object.keys(groups).some(u => u.includes('[2과목]'));
    if (hasSubject1 && hasSubject2) {
      console.log(`✅ [SUCCESS] Both 1과목 and 2과목 are successfully populated!`);
    } else {
      console.log(`❌ [FAILURE] Mismatch in 과목 classification. 1과목: ${hasSubject1}, 2과목: ${hasSubject2}`);
    }
  }

  if (sub.name === '승강기기능사') {
    const expected = ['01. 전기이론', '02. 기계일반', '03. 승강기 점검 및 보수', '04. 승강기 개론'];
    const missing = expected.filter(u => !groups[u]);
    if (missing.length === 0) {
      console.log(`✅ [SUCCESS] All elevator units are successfully populated!`);
    } else {
      console.log(`❌ [FAILURE] Missing units for Elevator: ${missing.join(', ')}`);
    }
  }
  ...
  if (sub.name === '정보처리기능사') {
    ...
    const missing = expected.filter(u => !groups[u]);
    if (missing.length === 0) {
      console.log(`✅ [SUCCESS] All information processing units are successfully populated!`);
    } else {
      console.log(`❌ [FAILURE] Missing units for Info Processing: ${missing.join(', ')}`);
    }
  }
});
