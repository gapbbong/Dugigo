...
// Fallback to hardcoded keys if .env.local is missing keys
if (!apiKey) {
...
async function generateSlideSet(subject, unit, setNum, questions) {
  ...
  if (fs.existsSync(targetPath)) {
    console.log(`  [Skip] ${subject} - ${unit} Set ${setNum} (Already exists)`);
    return;
  }
  ...
  while (retries > 0) {
    try {
      ...
      console.log(`  [SUCCESS] Generated & Cached: ${path.relative(process.cwd(), targetPath)}`);
      return;
    } catch (err) {
      console.warn(`  [RETRYING] Failed for ${unit} Set ${setNum}: ${err.message}`);
      await sleep(15000);
      retries--;
    }
  }
  console.error(`  [FAILED] Could not generate slides for ${unit} Set ${setNum}`);
}
...
async function main() {
  ...
  for (const sub of subjects) {
    ...
    for (const [unitName, questions] of Object.entries(grouped)) {
      ...
      console.log(`- Unit: ${unitName} (${questions.length} questions, ${totalSets} sets)`);

      for (let set = 1; set <= totalSets; set++) {
        const start = (set - 1) * pageSize;
        const chunk = questions.slice(start, start + pageSize);
        console.log(`  Generating Set ${set}/${totalSets}...`);
        await generateSlideSet(sub.name, unitName, set, chunk);
        
        // 12s sleep to strictly stay under 5 RPM Gemini limits
        await sleep(12000);
      }
    }
  }
  console.log('\n🎉 Slide Pre-generation Completed!');
}
