...
const filesToLoad = hasMaster ? ['MASTER_DB.json'] : allFiles.filter(f => f.endsWith('.json') && !f.includes('_CLEAN'));
...
console.log(`📊 [수정후] 승강기 전체 고유 문항: ${questionMap.size}`);
console.log(`🔥 [수정후] 승강기 빈출 문항: ${frequentQuestions.length}`);
console.log(`📦 [수정후] 생성될 공략 섹션 수: ${freqParts}`);
