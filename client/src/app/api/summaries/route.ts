...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    ` : subject.includes('컴퓨터활용능력') ? `
      [컴활 2급 전문가용 슬라이드 생성 규칙]
      1. **설명 수준 (Invisible Ease)**: 별도의 설명 없이도 중학생이 읽고 바로 이해할 수 있을 만큼 쉽고 친절한 용어를 사용하세요. 단, "중학생 수준"이라는 단어는 절대로 사용하지 마세요.
      2. **계층적 지식 전달**:
         - **[입문]**: 누구나 이해할 수 있는 쉬운 비유와 일상적인 언어로 개념 도입.
         - **[심화]**: 컴활 전문가다운 깊이 있는 기술적 보충 설명과 실무적 팁 제공.
         - **[핵심]**: 실제 문제 풀이 시 오답을 피하고 정답을 골라내는 '합격 꿀팁'과 '필수 암기 키워드'.
      3. **시각적 가이드 및 SVG**:
         - 엑셀 메뉴 경로나 단축키는 'Step-by-Step'으로 시각화하세요.
         - 'svg_overlay' 내에 한글이 포함될 경우, 반드시 표준 웹 폰트 설정을 고려하고 깨지지 않도록 텍스트 태그를 정밀하게 구성하세요.
      4. **비유의 마법**: 함수 설명 시 사장님이 강조하신 실생활 예시(VLOOKUP = 도서관 책 찾기 등)를 적극 활용하세요.
    ` : '';

    const prompt = `
      당신은 국내 최고의 IT 자격증 교육 전문가입니다. 
      제공된 문제를 바탕으로 '${subject}' 과목의 '${unit}' 단원 '${set}세트'를 위한 전문가급 학습 슬라이드 10장 내외를 생성하세요.

      ${subjectPrompt}

      [반드시 준수해야 할 응답 구조]
      1. **이미지 경로**: 'image' 필드는 "/summaries/${subject}/${safeUnitName}_${set}_slide_{id}.png" 형식으로 지정하세요.
      2. **콘텐츠 구성**: 각 슬라이드의 'content' 필드는 반드시 [쉬운 설명] -> [전문가 분석] -> [문제풀이 핵심] 순서로 내용을 꽉 채우세요.
      3. **SVG 품질**: 학습 보조 도구로서의 SVG 코드를 풍부하게 사용하고, 한글 텍스트 표현에 주의하세요.
        ...
        "slides": [
          {
            ...
            "svg_overlay": "<svg xmlns='http://www.w3.org/2000/svg' ...><!-- 한글 깨짐 방지 처리 --></svg>",
            "title": "슬라이드 제목",
            "content": "[쉬운 설명] ... \\n\\n[전문가 분석] ... \\n\\n[문제풀이 핵심] ...",
