    ...
    if (subject) {
      ...
      const classifyQuestion = (sub: string, q: any): string => {
        ...
        if (sub === '컴퓨터활용능력 2급') {
          ...
          if (isSubject1) {
            if (/윈도우|windows|바로 가기|제어판|탐색기|작업 표시줄|부팅|레지스트리|시스템 도구/.test(text)) return "[1과목] 운영체제(Windows) 활용";
            if (/폴더|파일|휴지통|속성|검색|옵션/.test(text)) return "[1과목] 폴더 및 파일 관리";
            if (/cpu|중앙처리장치|메모리|ram|rom|보조기억|ssd|hdd|usb|바이오스|bios|메인보드|입출력|칩셋/.test(text)) return "[1과목] 컴퓨터 하드웨어 시스템";
            if (/비트|바이트|워드|진법|자료|코드|ascii|unicode|유니코드|컴파일러|어셈블러/.test(text)) return "[1과목] 소프트웨어 및 데이터 표현";
            if (/멀티미디어|그래픽|이미지|동영상|사운드|오디오|코덱|비선형|bmp|jpg|png|gif|avi|mp4/.test(text)) return "[1과목] 멀티미디어 활용";
            if (/인터넷|url|ip|tcp|프로토콜|osi|브라우저|도메인|인트라넷|ftp|텔넷/.test(text)) return "[1과목] 인터넷 및 네트워크";
            if (/보안|바이러스|침해|암호|해킹|방화벽|변조|위조|iot|클라우드|인공지능|ai|모바일/.test(text)) return "[1과목] 정보 보안 및 ICT 기술";
            return "[1과목] 컴퓨터 일반 기타";
          } else {
            if (/셀 서식|사용자 정의|보호|시트|워크시트|통합 문서|데이터 입력|자동 채우기/.test(text)) return "[2과목] 워크시트 및 기본 작업";
            ...
            if (/부분합|피벗|시나리오|목표값|통합|데이터 표/.test(text)) return "[2과목] 데이터 분석 및 도구";
